...
public class AdminVoteResultsAdapter extends RecyclerView.Adapter<AdminVoteResultsAdapter.VoteResultViewHolder> {

    private final List<VoteResult> results;

    public AdminVoteResultsAdapter(List<VoteResult> results) {
        this.results = results;
    }

    @NonNull
    @Override
    public VoteResultViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_vote_result, parent, false);
        return new VoteResultViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull VoteResultViewHolder holder, int position) {
